...
export struct Glass {
  ...
  absorption: vec3f,
  /** The cross-section, wound counter-clockwise. */
  prismA: vec2f,
  prismB: vec2f,
  prismC: vec2f,
...
export fn glassEnvironment(
  ...
  let rotatedDirection = rotateEnvironmentDirection(direction, params.environmentRotation);
...
export fn glassEnvironmentLod(direction: vec3f, params: Glass) -> f32 {
  return env_lod(0.0, dpdx(direction), dpdy(direction), params.environmentTexelAngle);
}

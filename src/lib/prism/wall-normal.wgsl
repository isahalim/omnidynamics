...
fn wallNormalsFromSamples(
  ...
  let microNormalXy = (microMaterial.gb * 2.0 - 1.0) * microNormalStrength;
  return WallNormals(
...
export fn evaluateWallNormalsFromMaterial(
  ...
  let microMaterial = textureSampleBias(wallMaterial, materialSampler, microUv, -2.0);
...
/** Mip level for the material at this pixel, evaluated in uniform control flow. */
...
export fn evaluateWallNormalsLevel(
  ...
  let microMaterial = textureSampleLevel(
    wallMaterial,
    materialSampler,
    wallMaterialUv(worldPosition, materialWorldScale / frequency) + vec2f(0.371, 0.613),

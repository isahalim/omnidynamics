// Copied from vgpu's environment pyramid. This runs only while baking; runtime
// glass shading still performs one environment fetch.
import { PRISM_PI } from "./environment-map-common.wgsl";
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let sin_theta = max(sin(uv.y * PRISM_PI), 0.15);
  ...
  for (var index = 1; index < 3; index = index + 1) {
    sum += textureSampleLevel(src, src_samp, uv + step * offsets[index], 0.0) * weights[index];
    sum += textureSampleLevel(src, src_samp, uv - step * offsets[index], 0.0) * weights[index];
  }
  return sum;
}

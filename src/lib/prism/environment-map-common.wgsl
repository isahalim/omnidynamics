// Copied from vgpu's environment-map and transmission examples. Texture v = 0 is
...
export const PRISM_PI: f32 = 3.141592653589793;

export fn equirect_uv(direction: vec3f) -> vec2f {
  let d = normalize(direction);
  return vec2f(
    atan2(d.z, d.x) / (2.0 * PRISM_PI) + 0.5,
    acos(clamp(d.y, -1.0, 1.0)) / PRISM_PI,
  );
}

export fn direction_from_equirect(uv: vec2f) -> vec3f {
  let phi = (uv.x - 0.5) * 2.0 * PRISM_PI;
  let theta = uv.y * PRISM_PI;
  return vec3f(
    sin(theta) * cos(phi),
    cos(theta),
    sin(theta) * sin(phi),

// The projected spectral caustic: vgpu's 92,160-vertex light mesh rasterised
// over the wall, modulated by the plaster normal and the baked profile LUT.
import { Scene } from "./scene.wgsl";
import { beamWidthReveal } from "./beam-reveal.wgsl";
import { decodeLightVertex } from "./light-vertex.wgsl";
import { spectralSampleAt } from "./spectral.wgsl";
import { evaluateWallNormalsLevel, wallNormalTextureLod } from "./wall-normal.wgsl";
...
fn vs_main(
  ...
  if metadata.white == 0u && rawIntensity >= 0.0 {
    let spectral = spectralSampleAt(metadata.spectralIndex, scene.lightSpectralSamples);
...
fn wallNormalResponse(in: VertexOut) -> f32 {
  ...
  let worldTravel = worldPositionDx * travelGradient.x + worldPositionDy * travelGradient.y;
  ...
  let elevation = clamp(caustic.normalElevation, 1.0, 89.0) * 0.01745329252;
  ...
  let incidentLight = normalize(vec3f(rayDirection * cos(elevation), sin(elevation)));
  ...
  let relativeResponse = clamp(max(dot(normal, incidentLight), 0.0) / flatResponse, 0.0, 2.5);
  return mix(1.0, relativeResponse, clamp(caustic.normalInfluence, 0.0, 1.0));
}
...
fn fs_main(in: VertexOut) -> @location(0) vec4f {
  ...
  let widthReveal = beamWidthReveal(in.revealProfile, scene.beamWidthReveal);
  ...
  let outgoingFalloff = 1.0 / pow(
    1.0
      + max(scene.rainbowFalloffRate, 0.0)
        ...
        * max(in.travel, 0.0),
    max(scene.rainbowFalloffPower * max(caustic.falloffPowerScale, 0.0), 0.0001),
  );

// Outer/front interface of the prism.
//
// The material keeps a dielectric response: one refracted scene lookup, one
// studio reflection, Beer-Lambert absorption over the distance travelled inside
// the solid, and an additive HDR highlight so a bright studio panel keeps its
// shape on a low-IOR frontal face. Geometric antialiasing belongs to the 4x MSAA
// target.
//
// `sceneTexture` contains the wall, external light, the transparent back-side
// interface and internal light. The front shader follows air -> glass only as far
// as the first inner face and samples that resolved background there. The
// back-side material already owns glass -> air and TIR.
import { Glass, dielectricFresnel, glassEnvironment, glassEnvironmentLod } from "./glass-common.wgsl";
 ...
 * from the cross-section's edges and the two caps the extrusion added.
 */
...
fn traceInteriorHit(entryPosition: vec3f, insideDirection: vec3f) -> InteriorHit {
...
fn fs_main(in: VertexOut) -> @location(0) vec4f {
  ...
  let interiorHit = traceInteriorHit(in.worldPosition, insideDirection);
  let resolution = max(vec2f(textureDimensions(sceneTexture)), vec2f(1.0));
  ...
  // its radiance survives until the final ACES pass.

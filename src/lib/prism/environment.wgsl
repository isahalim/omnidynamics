// The deliberately sparse studio the prism reflects, as vgpu authors it: a dark
// back-left wall, a soft centre fill and a dominant cool right key. Baked once
// into the equirectangular HDR texture by `environment-bake.wgsl`.
//
// The final line replays the round trip the asset used to perform — encode to
// gamma 2.2, decode as sRGB — so the values a reflection reads here are the
// values a reflection reads there, including the small mismatch between those
// two curves.

import { srgbToLinear3 } from "./color.wgsl";
...
fn studioPanels() -> array<StudioPanel, 3> {
  return array<StudioPanel, 3>(
    ...
    // A broad, heavily feathered centre fill that barely lifts the bottom edge.
    StudioPanel(vec3f(0.0, -0.707, 0.707), vec2f(0.38, 0.62), 0.18, vec3f(1.0, 0.97, 0.91), 0.22),
    // The cool right panel is the dominant key and keeps a more defined edge.
    StudioPanel(vec3f(0.612, 0.354, 0.707), vec2f(0.5, 0.16), 0.035, vec3f(0.76, 0.88, 1.0), 20.0),
  );
}
...
/** Rotates a reflection into the studio's frame. */
